...
async function main() {
  ...
  server = net.createServer((connection) => {
    ...
    connection.on("error", (error) => {
      if (!shuttingDown) report(error.message);
    });
  ...
  child.once("error", (error) => {
    report(`could not start the rendering backend: ${error.message}`);
    void shutdown(1);
  });
  ...
  startupTimer = setTimeout(() => {
    report("rendering backend did not connect within 30 seconds.");
    void shutdown(1);
  }, 30_000);
  startupTimer.unref();
}
